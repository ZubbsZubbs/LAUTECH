...
app.get('/test-email', async (req, res) => {
    try {
        ...
        if (sendTest && diagnostics.emailConfigured) {
            ...
            const emailPromise = EmailService.sendEmail(process.env.EMAIL_USER, 'Test Email from Live Server', 'This is a test email to verify email configuration on production.', '<h2>Test Email</h2><p>This is a test email to verify email configuration on production.</p>');
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Email test timeout after 10 seconds')), 10000));
            try {
